...
Статусы актуализированы 2026-07-29 (v0.3.0). Репозиторий:
https://github.com/vladworldss/AI-Review-Bridge

## Development

- [x] `manifest_version: 3` — подтверждено в `build/chrome-mv3-prod/manifest.json`
- [x] Версия в манифесте актуальна — 0.3.0 (широкий хост); при новых изменениях
...
- [x] zip собирается из чистой директории без dev-файлов; env-подстановка
  ...
  в манифест (guard: единственный хост-паттерн `https://*/*`)
...
- [ ] **Smoke-тест пакета**: load unpacked из `build/chrome-mv3-prod`, открыть
  MR, синк + «Send to AI» работают, версия в шапке сайдбара = 0.3.0
- [ ] Финальный `./scripts/build-store-zip.sh` с коммита, который уйдёт в
  Store (текущий: `dist/gitlab-ai-review-bridge-v0.3.0.zip`)

## Listing / Review form (заполняется в кабинете Store)

- [x] Все permissions обоснованы в [listing.md](listing.md) — тексты готовы
- [x] Вопрос **B-1** закрыт: с 0.3.0 манифест объявляет `https://*/*`, чтобы
  расширение работало на корпоративных self-hosted GitLab
- [ ] **Broad host permission justification** вписан в форму ревью — текст
  готов в [listing.md](listing.md); ожидать более долгую проверку Store
